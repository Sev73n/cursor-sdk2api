...
| `/v1/models` | yes | BYOK returns one account catalog. Managed mode returns the union of exact catalog ids across the pool. Ids on the operator blocklist are omitted from this response; requests that already name a blocked id still run. |
